...
def setup_run_logging(log_dir: Path = None):
    ...
    _LOG_PATH = log_dir / f"PINN_ex1_{ts}.txt"
...
def pick_data_csv(cfg_row, set_id: int, data_type: str):
    ...
    if data_type == "hill":
        add_candidates(f"Data_Set{set_id}_HILL_ex1.csv", "Hill_trajectories_ex1.csv")
    elif data_type == "piecewise":
        add_candidates(f"piecewise_trajectories_ex1_set{set_id}.csv")
    elif data_type == "ramp":
        add_candidates(
            f"Ramp_trajectories_ex1_set{set_id}.csv",
            f"Data_Set{set_id}_RAMP_ex1.csv",
            "Ramp_trajectories_ex1.csv",
        )
    else:
        add_candidates("Hill_trajectories_ex1.csv")
    for p in candidates:
        if p.exists():
            return p
    return candidates[0]
...
class TwoNodeCyclePINN(nn.Module):
    def __init__(self, feature_map: FeatureMap, backbone: nn.Module,
        ...
        def to_raw(val_mat):
            ...
            arr = np.maximum(arr, self._param_eps)
            return torch.sqrt(torch.tensor(arr - self._param_eps, dtype=torch.float32))

        self.U_t = nn.Parameter(to_raw(U_np))
        self.L_t = nn.Parameter(to_raw(L_np))
        self.Th_t = nn.Parameter(to_raw(Th_np))
    ...
    def forward(self, t_norm, IC, inv_t_scale: float = 1.0):
        ...
        if self.ode_type == "ramp":
            r0 = -x0 + self.ramp(x1, U_pos[1, 0], L_pos[1, 0], Th_pos[1, 0], d[1, 0])
            r1 = -x1 + self.ramp(x0, U_pos[0, 1], L_pos[0, 1], Th_pos[0, 1], d[0, 1])
        else:
            r0 = -x0 + self.hill(x1, U_pos[1, 0], L_pos[1, 0], Th_pos[1, 0], d[1, 0])
            r1 = -x1 + self.hill(x0, U_pos[0, 1], L_pos[0, 1], Th_pos[0, 1], d[0, 1])
...
def train_once(cfg_row, results_dir: Path, row_index_for_errors=0, device=None, use_amp=False, amp_dtype_str="fp16"):
    ...
    if not data_path.exists():
        ...
        msg = [
            ...
            "Looked for:",
            f"  • Hill (new): {here / 'generated_data' / 'hill' / f'Data_Set{set_id}_HILL_ex1.csv'}",
            f"  • Hill:   {here / f'Data_Set{set_id}_HILL_ex1.csv'}",
            f"  • piecewise (new):  {here / 'generated_data' / 'piecewise' / f'piecewise_trajectories_ex1_set{set_id}.csv'}",
    ...
    for run_idx in range(1, n_runs + 1):
        ...
        for it in range(adam_steps):
            ...
            if it % 500 == 0:
                ...
                msg = (f"[ex1 set {set_id} run {run_idx}] Adam {it:05d} | Total {loss.item():.3e} | "
                       ...
                       f"U_01={U_now[0,1]:.4f}, U_10={U_now[1,0]:.4f} | "
                       f"L_01={L_now[0,1]:.4f}, L_10={L_now[1,0]:.4f} | "
                       f"Th_01={Th_now[0,1]:.4f}, Th_10={Th_now[1,0]:.4f} | "
        ...
        try:
            log(f"[learned params] U_01={U_learned[0,1]:.4f}, U_10={U_learned[1,0]:.4f} | "
                f"L_01={L_learned[0,1]:.4f}, L_10={L_learned[1,0]:.4f} | "
                f"Th_01={Th_learned[0,1]:.4f}, Th_10={Th_learned[1,0]:.4f} | "
                f"d_01={d_learned[0,1]:.4f}, d_10={d_learned[1,0]:.4f}")
        except Exception:
            pass
        ...
        plt.savefig(plots_dir / f"x0_ex1_set{set_id}_run{run_idx}.png", dpi=150); plt.close()
        ...
        plt.savefig(plots_dir / f"x1_ex1_set{set_id}_run{run_idx}.png", dpi=150); plt.close()
...
def main():
    ...
    if "hill" in present_types:
        ...
        try_run_generator("Hill_Data_production_ex1.py", [params_csv])
