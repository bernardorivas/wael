...
PARAM_PATH = Path(sys.argv[1]) if len(sys.argv) > 1 else BASE_DIR / "param_ex1.csv"
...
for k, row in hill_rows.iterrows():
    ...
    try:
        D_12 = float(row["D12_input"])
    except Exception:
        D_12 = 12
    try:
        D_21 = float(row["D21_input"])
    except Exception:
        D_21 = 12
    ...
    plot_path = PLOT_DIR / f"Set{set_num}_TrajectoryHILL_ex1.png"
    csv_path  = DATA_DIR / f"Data_Set{set_num}_HILL_ex1.csv"
    ...
    if add_noise_flag in ("yes", "y", "true", "1"):
        try:
            noise_ub_percent = float(row.get("noise_ub"))
        except Exception:
            raise ValueError(f"add_noise requested for CSV row {csv_row} but noise_ub is missing or invalid.")
        if noise_ub_percent < 0:
            raise ValueError(f"noise_ub (percent) must be non-negative for CSV row {csv_row}; got {noise_ub_percent}")
        ...
        df_out['noise_ub_percent'] = noise_ub_percent
        df_out['noise_ub_abs'] = noise_ub_abs
        ...
        print(
            ...
            f"(abs ub={noise_ub_abs:.6g}) to {n_rows} rows; "
            f"applied post-noise clipping at 0"
        )
    else:
        df_out['add_noise'] = 'no'
        df_out['noise_ub_percent'] = float(row.get("noise_ub", 0.0) or 0.0)
    ...
    if add_noise_flag in ("yes", "y", "true", "1"):
        noisy_plot_path = PLOT_DIR / f"Set{set_num}_TrajectoryHILL_ex1_noisy.png"
