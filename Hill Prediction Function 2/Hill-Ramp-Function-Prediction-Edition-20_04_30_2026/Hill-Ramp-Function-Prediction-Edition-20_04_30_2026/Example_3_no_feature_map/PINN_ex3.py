...
GENERATED_DATA_SUBDIRS = {
    "hill": GENERATED_DATA_DIR / "hill",
    "piecewise": GENERATED_DATA_DIR / "piecewise",
    "ramp": GENERATED_DATA_DIR / "ramp",
}
...
def setup_run_logging(log_dir: Path = None):
    ...
    _LOG_PATH = log_dir / f"PINN_ex3_{ts}.txt"
...
def parse_mat2x2(row, name, row_idx=0):
    ...
    if A.shape != (3, 3):
        raise ValueError(f"{name} must be a 3x3 matrix on row {row_idx}")
    return A
...
def pick_data_csv(cfg_row, set_id: int, data_type: str):
    ...
    candidates = []
    search_roots = []
    preferred_dir = GENERATED_DATA_SUBDIRS.get(data_type)
    ...
    if data_type == "hill":
        add_candidates(f"Data_Set{set_id}_HILL_ex3.csv", "Hill_trajectories_ex3.csv")
    elif data_type == "piecewise":
        add_candidates(f"piecewise_trajectories_ex3_set{set_id}.csv")
    elif data_type == "ramp":
        add_candidates(
            f"Ramp_trajectories_ex3_set{set_id}.csv",
            f"Data_Set{set_id}_RAMP_ex3.csv",
            "Ramp_trajectories_ex3.csv",
        )
    else:
        add_candidates("Hill_trajectories_ex3.csv")
    for p in candidates:
        if p.exists():
            return p
    return candidates[0]
...
class ThreeNodeCyclePINN(nn.Module):
    def __init__(self, feature_map: FeatureMap, backbone: nn.Module,
                 U_init, L_init, Th_init, D_init,
                 ic_embed_dim: int = 0,
                 ode_type: str = "hill"):
        super().__init__()
        ...
        U_init_t = torch.tensor(U_init, dtype=torch.float32)
        L_init_t = torch.tensor(L_init, dtype=torch.float32)
        Th_init_t = torch.tensor(Th_init, dtype=torch.float32)
        self._param_eps = 1e-6

        def to_raw(init_t):
            positive_init = torch.where(init_t != 0, torch.tensor(1.0), torch.tensor(0.0))
            positive_init = torch.clamp(positive_init, min=self._param_eps)
            return torch.sqrt(positive_init - self._param_eps)

        self.U_t = nn.Parameter(to_raw(U_init_t))
        self.L_t = nn.Parameter(to_raw(L_init_t))
        self.Th_t = nn.Parameter(to_raw(Th_init_t))



        d_tensor = torch.tensor(D_init, dtype=torch.float32)

        d_tensor = torch.where(d_tensor != 0, torch.tensor(10.0), d_tensor)

        d_tensor = torch.clamp(d_tensor, min=1e-3)
        self.logd = nn.Parameter(torch.log(d_tensor))

        self.ic_embed = None
        if ic_embed_dim > 0:
            self.ic_embed = nn.Sequential(
                nn.Linear(3, ic_embed_dim), nn.Tanh(),
                nn.Linear(ic_embed_dim, 3)
            )
    ...
    def forward(self, t_norm, IC, inv_t_scale: float = 1.0):
        ...
        dx2_dt_norm = torch.autograd.grad(x2.sum(), t_norm, create_graph=True)[0]
        x0_t = inv_t_scale * dx0_dt_norm
        x1_t = inv_t_scale * dx1_dt_norm
        x2_t = inv_t_scale * dx2_dt_norm
        ...
        if self.ode_type == "ramp":
            f20 = self.ramp(x2, U_pos[2, 0], L_pos[2, 0], Th_pos[2, 0], d[2, 0])
            f01 = self.ramp(x0, U_pos[0, 1], L_pos[0, 1], Th_pos[0, 1], d[0, 1])
            f12 = self.ramp(x1, U_pos[1, 2], L_pos[1, 2], Th_pos[1, 2], d[1, 2])
        else:
            f20 = self.hill(x2, U_pos[2, 0], L_pos[2, 0], Th_pos[2, 0], d[2, 0])
            f01 = self.hill(x0, U_pos[0, 1], L_pos[0, 1], Th_pos[0, 1], d[0, 1])
            f12 = self.hill(x1, U_pos[1, 2], L_pos[1, 2], Th_pos[1, 2], d[1, 2])
...
def train_once(cfg_row, results_dir: Path, row_index_for_errors=0, device=None, use_amp=False, amp_dtype_str="fp16"):
    ...
    try:
        D_np = np.array(ast.literal_eval(cfg_row["D_np"]), dtype=float)
        if D_np.shape != (3, 3):
             raise ValueError(f"D_np must be 3x3 on row {row_index_for_errors}")
    except Exception:
        D_np = np.zeros((3, 3))

        D_np[0,1] = 15; D_np[1,2] = 15; D_np[2,0] = 15
    ...
    if not data_path.exists():
        here = BASE_DIR
        hill_default = GENERATED_DATA_SUBDIRS["hill"] / f"Data_Set{set_id}_HILL_ex3.csv"
        piecewise_default = GENERATED_DATA_SUBDIRS["piecewise"] / f"piecewise_trajectories_ex3_set{set_id}.csv"
        ramp_default = GENERATED_DATA_SUBDIRS["ramp"] / f"Data_Set{set_id}_RAMP_ex3.csv"
        msg = [
            f"Data file not found: {data_path}",
            f"Generator cwd: {here}",
            "Looked for:",
            f"  • Hill:      {hill_default}",
            f"  • piecewise: {piecewise_default}",
            f"  • Ramp:      {ramp_default}",
            "  • Legacy root-level files are also checked as a fallback.",
            "Or set a custom path via the 'data_csv' column in param_ex3.csv."
        ]
    ...
    if data_type == "piecewise":
        if "x0" not in df.columns and {"x", "y", "z"}.issubset(df.columns):
            df = df.rename(columns={"x": "x0", "y": "x1", "z": "x2"})

    for col in ["Time", "x0", "x1", "x2"]:
    ...
    if "Trajectory" in df.columns:
        ...
        ic_pick = ic_df.groupby("Trajectory", as_index=False).first()[["Trajectory", "x0", "x1", "x2"]]
        ic_pick = ic_pick.rename(columns={"x0": "ic0", "x1": "ic1", "x2": "ic2"})
        df = df.merge(ic_pick, on="Trajectory", how="left")
    else:
        first_idx = df["Time"].idxmin()
        ic0, ic1, ic2 = float(df.loc[first_idx, "x0"]), float(df.loc[first_idx, "x1"]), float(df.loc[first_idx, "x2"])
        df["Trajectory"] = 1; df["ic0"] = ic0; df["ic1"] = ic1; df["ic2"] = ic2
    ...
    if corners_only and data_type in {"ramp", "piecewise"}:
        ...
        angle_deg_thresh = 8.0
        pad_points = 2
        log(f"[preproc] turn-keeper for {data_type}... (angle_deg_thresh={angle_deg_thresh}°, pad={pad_points})")
        before = len(df)
    ...
    ic_all      = torch.tensor(df[["ic0", "ic1", "ic2"]].values,   dtype=torch.float32, device=device)
    ...
    x2_all      = torch.tensor(df["x2"].values[:, None],    dtype=torch.float32, device=device)
    N_total = t_all_norm.size(0)

    ensure_dir(results_dir)
    plots_dir = results_dir / "plots_ex3"
    ...
    for run_idx in range(1, n_runs + 1):
        ...
        log(f"[model] backbone={model_type} in_dim={in_dim} out_dim=3 width={hidden_width} depth={hidden_depth}")
        backbone = build_model(model_type, in_dim, 3, hidden_width, hidden_depth, omega0).to(device)

        model = ThreeNodeCyclePINN(
            feature_map=fmap, backbone=backbone,
            U_init=U, L_init=L, Th_init=Th, D_init=D_np,
        ...
        last_good_logd = model.logd.detach().clone()
        ...
        for it in range(adam_steps):
            ...
            if device.type == "cuda" and use_amp:
                with torch.cuda.amp.autocast(dtype=amp_dtype):
                    x0_p, x1_p, x2_p, f0, f1, f2 = model(t_in_norm, ic_train, inv_t_scale=inv_t_scale)
                    loss_data = ((x0_train - x0_p)**2 + (x1_train - x1_p)**2 + (x2_train - x2_p)**2).mean()
                    loss_res  = (f0**2 + f1**2 + f2**2).mean()
                    if ic_mask.any():
                        ic_err = (x0_p[ic_mask] - ic_train[ic_mask, 0:1]).pow(2).mean() + \
                                 (x1_p[ic_mask] - ic_train[ic_mask, 1:2]).pow(2).mean() + \
                                 (x2_p[ic_mask] - ic_train[ic_mask, 2:3]).pow(2).mean()
                        loss_ic = ic_err
                    else:
            ...
            try:
                ...
                if not (loss_finite and data_finite and res_finite and ic_finite):
                    ...
                    with torch.no_grad():
                        model.logd.copy_(last_good_logd)
                    nan_detected = True
            ...
            if it % 500 == 0:
                d_now = torch.exp(model.logd).detach().cpu().numpy()
                U_now, L_now, Th_now = [m.detach().cpu().numpy() for m in model.pos_matrices()]
                msg = (f"[ex3 set {set_id} run {run_idx}] Adam {it:05d} | Total {loss.item():.3e} | "
                       f"Data {loss_data.item():.3e}(w={data_loss_wt}) | Res {loss_res.item():.3e}(w={physics_loss_wt}) | IC {loss_ic.item():.3e}(w={ic_loss_wt}) | "
                       f"U: [2,0]={U_now[2,0]:.4f}, [0,1]={U_now[0,1]:.4f}, [1,2]={U_now[1,2]:.4f} | "
                       f"L: [2,0]={L_now[2,0]:.4f}, [0,1]={L_now[0,1]:.4f}, [1,2]={L_now[1,2]:.4f} | "
                       f"Th: [2,0]={Th_now[2,0]:.4f}, [0,1]={Th_now[0,1]:.4f}, [1,2]={Th_now[1,2]:.4f} | "
                       f"d: [2,0]={d_now[2,0]:.4f}, [0,1]={d_now[0,1]:.4f}, [1,2]={d_now[1,2]:.4f}")
        ...
        if lbfgs_on:
            ...
            def closure():
                ...
                try:
                    ...
                    if not (loss_finite and data_finite and res_finite and ic_finite):
                        ...
                        if last_lbfgs_d is None:
                            try:
                                last_lbfgs_d = torch.exp(last_good_logd).detach().cpu().numpy()
                            except Exception:
                                last_lbfgs_d = None
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                d_tmp = torch.exp(model.logd).detach().cpu().numpy()
                ...
                U_now, L_now, Th_now = [m.detach().cpu().numpy() for m in model.pos_matrices()]
                msg = (f"[ex3 set {set_id} run {run_idx}] LBFGS loss: {loss.item():.3e} | "
                       f"Data {loss_data.item():.3e}(w={data_loss_wt}) | Res {loss_res.item():.3e}(w={physics_loss_wt}) | IC {loss_ic.item():.3e}(w={ic_loss_wt}) | "
                       f"U: [2,0]={U_now[2,0]:.4f}, [0,1]={U_now[0,1]:.4f}, [1,2]={U_now[1,2]:.4f} | "
                       f"d: [2,0]={d_now[2,0]:.4f}, [0,1]={d_now[0,1]:.4f}, [1,2]={d_now[1,2]:.4f}")
                if learn_feature_params:
        ...
        with torch.enable_grad():
            x0_pred, x1_pred, x2_pred, _, _, _ = model(t_all_req_norm, ic_all, inv_t_scale=inv_t_scale)
        err0 = torch.norm(x0_all - x0_pred.detach()) / torch.norm(x0_all)
        err1 = torch.norm(x1_all - x1_pred.detach()) / torch.norm(x1_all)
        err2 = torch.norm(x2_all - x2_pred.detach()) / torch.norm(x2_all)
        ...
        try:
            ...
            try:
                d_learned = torch.exp(last_good_logd).detach().cpu().numpy()
            except Exception:

                d_learned = np.full((3, 3), float('nan'))
        ...
        def _get_param_matrix(name: str, clamp_positive: bool = False):
            try:
                if last_lbfgs_state is not None and name in last_lbfgs_state:
                    ...
                    return np.full((3, 3), float('nan'), dtype=float)
                if clamp_positive:
                    tensor_val = tensor_val**2 + getattr(model, "_param_eps", 1e-6)
        ...
        try:
            log(f"[learned params final] "
                f"U: [2,0]={U_learned[2,0]:.4f}, [0,1]={U_learned[0,1]:.4f}, [1,2]={U_learned[1,2]:.4f} | "
                f"d: [2,0]={d_learned[2,0]:.4f}, [0,1]={d_learned[0,1]:.4f}, [1,2]={d_learned[1,2]:.4f}")
        except Exception:
            pass
        ...
        if "Trajectory" in df.columns:
            for tr in sorted(df['Trajectory'].unique()):
                ...
                ic_pair = sub[['ic0','ic1','ic2']].iloc[0].values.astype(np.float32)
                ...
                with torch.enable_grad():
                    x0_d, x1_d, x2_d, _, _, _ = model(t_dense_t, ic_dense_t, inv_t_scale=inv_t_scale)
                dense_preds[tr] = {
                    "t": t_dense,
                    "x0": x0_d.detach().cpu().numpy().ravel(),
                    "x1": x1_d.detach().cpu().numpy().ravel(),
                    "x2": x2_d.detach().cpu().numpy().ravel(),
                }
        else:
            ...
            t_dense = np.linspace(t_start, t_end, n_dense)
            t_dense_norm = (t_dense - t_min) / t_scale
            ic_pair = df[['ic0','ic1','ic2']].iloc[0].values.astype(np.float32)
            t_dense_t = torch.tensor(t_dense_norm[:, None], dtype=torch.float32, device=device).requires_grad_(True)
            ic_dense_t = torch.tensor(np.repeat(ic_pair[None, :], n_dense, axis=0), dtype=torch.float32, device=device)
            with torch.enable_grad():
                x0_d, x1_d, x2_d, _, _, _ = model(t_dense_t, ic_dense_t, inv_t_scale=inv_t_scale)
        ...
        x2p = x2_pred.detach().cpu().numpy().ravel()
        ...
        plt.savefig(plots_dir / f"x0_ex3_set{set_id}_run{run_idx}.png", dpi=150); plt.close()
        ...
        plt.savefig(plots_dir / f"x1_ex3_set{set_id}_run{run_idx}.png", dpi=150); plt.close()

        plt.figure()
        plt.plot(times, df["x2"].values, '.', label='x2 true', alpha=0.6)
        if "Trajectory" in df.columns:
            for tr in sorted(df['Trajectory'].unique()):
                mask = (df['Trajectory'].values == tr)
                dp = dense_preds.get(tr)
                if dp is not None:
                    plt.plot(dp['t'], dp['x2'], '-', linewidth=1.2, label=f'pred tr{tr}')
                else:
                    plt.plot(df.loc[mask, 'Time'].values, x2p[mask], '-', linewidth=1)
        else:
            dp = dense_preds.get(1)
            if dp is not None:
                plt.plot(dp['t'], dp['x2'], '-', linewidth=1.2, label='pred')
            else:
                plt.plot(times, x2p, '-', linewidth=1)
        plt.legend(); plt.xlabel('Time'); plt.ylabel('x2'); plt.tight_layout()
        plt.savefig(plots_dir / f"x2_ex3_set{set_id}_run{run_idx}.png", dpi=150); plt.close()
        ...
        row_dict = dict(row_copy) | dict(ic_seed=ic_seed_in_data,
            ...
            U_01=float(U_learned[0,1]), U_12=float(U_learned[1,2]), U_20=float(U_learned[2,0]),
            L_01=float(L_learned[0,1]), L_12=float(L_learned[1,2]), L_20=float(L_learned[2,0]),
            Th_01=float(Th_learned[0,1]), Th_12=float(Th_learned[1,2]), Th_20=float(Th_learned[2,0]),
            d_01=float(d_learned[0,1]), d_12=float(d_learned[1,2]), d_20=float(d_learned[2,0]),

            rel_err_x0=float(err0.item()), rel_err_x1=float(err1.item()), rel_err_x2=float(err2.item()),
...
def main():
    parser = argparse.ArgumentParser(description="Unified PINN ex3 runner (merged) with GPU/AMP options")
    parser.add_argument("params_csv", nargs="?", default="param_ex3.csv", help="Parameter CSV path")
    ...
    if not params_csv_path.is_absolute():
        params_csv_path = BASE_DIR / params_csv_path
    ...
    if not params_csv_path.exists():
        raise FileNotFoundError(f"Params CSV not found: {params_csv_path.resolve()}")

    cfg_df = pd.read_csv(params_csv)
    results_dir = BASE_DIR / "results_three_node_cycle_ex3"; ensure_dir(results_dir)
    results_path = results_dir / "results_ex3.csv"
    old_results_path = results_dir / "runs_ex3.csv"
    if (not results_path.exists()) and old_results_path.exists():
        try:
            ...
            log("[results] Migrated existing runs_ex3.csv to results_ex3.csv")
        except Exception as e:
