...
def PiecewiseFunction(a, b, theta, d):

    _ = d
    return lambda x: a if x <= theta else b

def piecewise_ode(t, y, U, L, T, D):
    piece00 = PiecewiseFunction(L[0,0], U[0,0], T[0,0], D[0,0])
    piece01 = PiecewiseFunction(U[0,1], L[0,1], T[0,1], D[0,1])
    piece10 = PiecewiseFunction(L[1,0], U[1,0], T[1,0], D[1,0])
    piece11 = PiecewiseFunction(L[1,1], U[1,1], T[1,1], D[1,1])

    x0, x1 = y
    dx0 = -x0 + piece00(x0) + piece10(x1)
    dx1 = -x1 + piece01(x0) * piece11(x1)
    return [dx0, dx1]
...
def piecewise_data_production_ex2():
    ...
    output_dir = os.path.join(base_dir, 'generated_data_ex2', 'piecewise')
    os.makedirs(output_dir, exist_ok=True)

    param_path = os.path.join(base_dir, 'param_ex2.csv')
    logmsg("Starting piecewise data production (ex2)...")
    t_global = time.time()

    if not os.path.isfile(param_path):
        raise FileNotFoundError(f"CSV file not found: {param_path}")

    logmsg("Loading parameters from 'param_ex2.csv'")
    ...
    print(f"Found {len(piecewise_rows)} piecewise set(s) in 'param_ex2.csv'.")
    ...
    for r, row_data in piecewise_rows.iterrows():
        ...
        D[0, 0] = get_or_default(Tcsv, r, "D11_input", 12.0, dtype=float)
        D[0, 1] = get_or_default(Tcsv, r, "D12_input", 12.0, dtype=float)
        D[1, 0] = get_or_default(Tcsv, r, "D21_input", 12.0, dtype=float)
        D[1, 1] = get_or_default(Tcsv, r, "D22_input", 12.0, dtype=float)
        ...
        plot_path = os.path.join(output_dir, f'Set{set_id}_TrajectoryPIECEWISE_ex2.png')
        fig.savefig(plot_path, dpi=150)
        plt.close(fig)
        print(f"  • Saved plot -> {os.path.relpath(plot_path)}")

        logmsg(f"Set {set_id}: writing per-set CSV")
        per_set_csv_path = os.path.join(output_dir, f'Data_Set{set_id}_PIECEWISE_ex2.csv')
        if all_traj_data_list:
            ...
            noise_ub = get_or_default(Tcsv, r, "noise_ub", 0.0, dtype=float)

            if add_noise and noise_ub > 0:
                ...
                noise = rng.uniform(-1.0, 1.0, size=(len(df_out), 2)) * noise_amp
                ...
                print(f"  Added bounded uniform observation noise with ub={noise_ub}% to {len(df_out)} rows (clipped at 0)")
            else:
                df_out['add_noise'] = 'no'
                df_out['noise_ub'] = float(noise_ub)
            ...
            if add_noise and noise_ub > 0:
                noisy_plot_path = os.path.join(output_dir, f'Set{set_id}_TrajectoryPIECEWISE_ex2_noisy.png')
                try:
    ...
    print("\nAll piecewise sets processed for exercise 2.")
    logmsg(f"Total wall time: {time.time() - t_global:5.2f}s")

if __name__ == '__main__':
    piecewise_data_production_ex2()
